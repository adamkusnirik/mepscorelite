...
GITHUB_URL_PUBLIC="https://github.com/adamkusnirik/mepscore.git"
...
# Step 4: Clone from GitHub and checkout specific commit
print_step "4. Cloning fresh code from GitHub..."
cd $APP_DIR

# Try cloning - if it fails, try without auth
if ! git clone $GITHUB_URL . 2>/dev/null; then
    print_warning "HTTPS clone failed, trying public repository access..."
    # Set git to use public access for this repo
    git config --global url."https://github.com/".insteadOf git@github.com:
    git clone $GITHUB_URL_PUBLIC .
fi

# Checkout specific commit if provided
SPECIFIC_COMMIT="3be6a36adf4ef090715b6f1282c9aa15691ddd80"
if [ ! -z "$SPECIFIC_COMMIT" ]; then
    print_step "4a. Checking out specific commit: $SPECIFIC_COMMIT"
    git checkout $SPECIFIC_COMMIT
    print_status "Checked out commit: $SPECIFIC_COMMIT"
fi
